		...
		while (playAgain) {
			...
			while (status == GameStatus.IN_PROGRESS) {
				...
				try {
					...
					System.out.println(currentPlayer.getName() + "'s (" + currentPlayer.getSymbol() + ") turn number: "
							+ moveNumber);
					...
					// Input Validation
					if (row < 0 || row > 2 ) {
						System.out.println("Invalid Input! Please enter a valid row  between 0-2.");
						continue;
					}
					if(( col < 0 || col > 2)) {
						System.out.println("Invalid Input! Enter Correct Column between 0-2 ");
						continue;
					}
